...
## Governance pillar (this session) -- first of 8 pillar forms

`/admin/config-engine/governance`. Four sections from the mockup:

- **Government Authorities**, **Sector Stakeholders** -- real, normalized
  tables (`country_authorities`, `country_stakeholders`), because the
  mockup says explicitly these are cross-pillar reference data
  ("Authorities registered here can be granted approval rights in
  Procurement, Investment, Sustainability and ICV workflows"). Same
  reasoning as `country_ftas`/`country_hs_codes` -- not governance-only
  settings, so not nested in this pillar's own jsonb payload.
- **Escalation Matrix**, **Data Governance & Access Policy** --
  genuinely governance-specific, so these live in `pillar_configs.payload`
  where `pillar='governance'`, exactly the column this module's schema
  was built for from the start.

**The generalizable part of this session**: `getCountryPillarPayload<T>()`
/ `updateCountryPillarPayload()` in the adapter are deliberately generic
across all 8 pillars, not Governance-specific -- every remaining pillar
(Procurement, B2B, Import, Export, Investment, Sustainability, ICV) reuses
this exact pair of functions for its own country-level admin edit path.
Only each pillar's own Zod schema differs. This is distinct from
`resolve_pillar_config()` (the SQL function from the foundation
migration) -- that's the company-facing, possibly-state-delegated read
path; this pair is the admin's own country-level write path.

Ref codes for authorities/stakeholders (`A001`, `S001`, ...) are
generated by counting existing rows at insert time, not a Postgres
sequence -- not perfectly race-safe under concurrent inserts, acceptable
for one Country Admin's actual usage pattern, flagged rather than
silently assumed fine forever.

Verified against real Postgres before any UI was written:
`tests/db/governance.test.sql` (6 assertions, including confirming
`resolve_pillar_config()` returns a realistic governance payload shape
