...
**Build:** the hierarchical config model: Country (National Cluster) → State (State Cluster), each carrying identity (currency, tax, dial code, financial year), FTAs, thrust sectors, tiered support partners, and the four authority types (Procurement/Investment/Trade/Customs), plus templating ("Configuration Templates → Apply Template") so a new country isn't configured from a blank page. Central Config vs delegated State Config toggle per pillar.
...
**Definition of Done:** a country can be fully configured through the UI, a state can be configured under it (inheriting or overriding per pillar), and at least one other module (master data, below) successfully reads a config value from it.
