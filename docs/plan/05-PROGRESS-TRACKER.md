...
Last updated: 2026-09-19 (Country Identity UI reachable end to end - auth, login, and a real db-splitting bug fixed)
...
| 0.5 | Country/State Config Engine | In progress | Schema/RBAC/resolver/approval-workflow done and confirmed live. Country Identity UI built and now genuinely reachable end to end: added `proxy.ts` (retroactive Phase 0 gap — session-refresh middleware, renamed from `middleware.ts` per Next.js 16's actual convention), a temporary `/login` page (explicitly not Phase 2 — just enough to test with a Supabase user created directly in the dashboard), and a one-time seed script (`docs/modules/config-engine/seed-test-admin.md`). Fixed a real build-breaking bug along the way: `lib/db/client.ts` mixed server-only (`next/headers`) and browser-safe code in one file, which broke the build the moment a Client Component needed `getBrowserDb()`. Split into `client.ts` (server-only, hardened with the `server-only` package) and `browser-client.ts`. All caught and fixed via `npm run build` on a fresh clone before reaching the repo. **Remaining UI: Master Data, State Cluster management, and all 8 pillar forms.** | Who reviews/approves a country's own submission (no platform-admin role yet, service-role-only for now). |
